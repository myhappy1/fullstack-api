...
saved_model_fname = "model/frinallized_model.sav"
...
def calculate_item_based(item_id, items):
    ...
    recs =  loaded_model.similar_item(item_id=int(item_id),N=11)
...
def calculate_user_based(user_items, items):
    ...
    recs =  loaded_model.recommend(
        user_id = 0,user_items=user_items,recalculate_user = True,N=10
    )
    return [str(items[r] for r in recs[0])]
...
def item_based_recommendation(item_id):
    ...
    result_items = movies_df[movies_df['movieId'].isin(result)].to_dict("record")
    return result_items
